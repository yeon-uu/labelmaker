...
이 프로젝트는 프론트 구현을 Claude Code로 오케스트레이션하고, 컨셉과 디자인 방향, 품질 판정을 직접 맡아 배포까지 완성했다. 화면을 브라우저로 열어 좌표와 픽셀을 수치로 확인하며 다듬었다. 그중 기술적으로 유효했던 두 가지.
...
- **성과**: 한글 48/48, 영문 50/50 히트존이 키캡에 정확히 안착(100%). 한글, 영문, 이모지 3종 키보드 이미지를 같은 방식으로 정렬.

### ② 이모지 키 매핑을 원본 대조로 검증

- **문제**: 반짝이 키 개수를 하나 잘못 세면서 배열이 한 칸씩 밀려 리본 키가 사라졌다.
- **해결**: 스프라이트 17종과 키보드 이미지를 나란히 열어 하나씩 대조하고 매핑을 다시 작성.
- **성과**: 이모지 키 25개가 눌린 그림과 삽입 스프라이트 100% 일치. 중복 키 8개는 미니(0.65배) 크기로 분리해 같은 그림도 다른 결과를 내게 함.

배포는 GitHub Pages로 붙였다. 정적 파일이라 서버 없이 무료로 올라가고, 커밋할 때마다 자동 갱신된다.
