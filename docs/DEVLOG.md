...
## 2026-07-05(12차) — 한글 이미지 교체(kr_trim 재트림) + R1~R5 좌표 전면 재측정 + ▲▼ 히트존 분리

### 배경
새 소스 이미지 `assets/한글머신오류고침.png`(1535×1024, RGB, 알파 없음)로 `assets/kr_trim.png`를 교체하라는 지시. 기존 kr_trim(1096×792)과 달리 이 이미지는 R2에 **ㅔ 키가 실제로 존재**(11차 이전까지 "이 기기엔 ㅔ가 없다"는 게 확정 사실이었는데, 새 이미지에서는 있음)하고, R4는 콤마가 실제로 2개 그려져 있다. 또한 사용자가 "▲/▼가 세로로 겹쳐 배치돼 ▼가 안 눌렸다"는 버그를 지적.

### 1. 트림
배경색(크림, 코너 샘플 기준) 거리 임계값(12) 기반 alpha-bbox 방식으로 몸체만 크롭 — Python PIL, 원본 보존, `assets/kr_trim.png`에 덮어씀. 결과 **1109×794**(기존 1096×792과 비슷한 크기, 비율 1.3967 vs 기존 1.3838).

### 2. 격자 재측정 방법
지시대로 "행 밴드·열 밴드를 한 번에 검출"하는 방식 사용, 픽셀 하나하나 경계 무한 탐색 안 함:
- **행 밴드**: 크림 키캡 vs 노란 데크 B채널 구분(`b<210 and r>195`)으로 전체 폭 다수결(deck fraction) y별로 스캔, local peak 쌍(각 행 사이 이중 안티에일리어싱 라인)의 중심을 행 경계로 확정. R1 top 40.554% h 9.950%, R2 top 51.511% h 8.438%, R3 top 60.957% h 8.438%, R4 top 70.277% h 8.438%, R5 top 79.849% h 9.950%.
- **열 밴드**: 각 행의 좌우 프레임 경계(첫 키 왼쪽/마지막 키 오른쪽)를 다수결로 실측(R1/R2/R4는 58~59px, R3도 58px 시작, 우측 전부 1051px)한 뒤, **실제 관찰된 칸 수로 균등분할**(자동 gap-detection이 라운드 코너 안티에일리어싱 노이즈로 계속 오검출돼 시간이 걸려, 균등분할 후 오버레이 이미지를 Read로 직접 열어 육안 대조하는 방식으로 전환 — 지시받은 "무한 탐색 금지" 취지에 맞춤). 오버레이 검증 결과 전 행에서 격자선이 키 사이 간격과 잘 일치함을 확인.
- **R5(방향키/기능키)**: 한/영/Space/◀/▲▼/▶/선택줄변경 6그룹 각각의 실제 좌우 경계를 개별 실측(▲▼ 칼럼은 ◀▶의 세로 중간부가 아니라 ▲ 자신의 상단 y범위에서 스캔해야 좌우 gap이 보임 — 처음 시도한 y범위는 ▲▼ 사이 연결부라 gap이 안 잡혔음, y범위를 ▲ 버튼 자체 높이로 좁혀 해결).

### 3. R1~R4 최종 확정 좌표/매핑 (kr_trim.png 1109×794 기준 %)
- **R1**(top 40.554% h 9.950%, 11칸, 균등분할 left 5.275%~93.09%): `1 2 3 4 5 6 7 8 9 0 del_cancel`, 각 width 8.136%.
- **R2**(top 51.511% h 8.438%, 11칸): `ㅂ ㅈ ㄷ ㄱ ㅅ ㅛ ㅕ ㅑ ㅐ ㅔ emoji`, 각 width 8.136%. **ㅔ가 새로 추가된 자리**(ㅐ 다음, 이모티콘 앞).
- **R3**(top 60.957% h 8.438%, 10칸): `ㅁ ㄴ ㅇ ㄹ ㅎ ㅗ ㅓ ㅏ ㅣ backspace`, 각 width 8.950%.
- **R4**(top 70.277% h 8.438%, 11칸): `shift ㅋ ㅌ ㅊ ㅍ ㅠ ㅜ ㅡ , , .`, 각 width 8.136%. **콤마가 실제로 2칸**(사진 그대로, 둘 다 `,` 키로 매핑).
- **R5**(top 79.849% h 9.950%): `lang_toggle`(left 5.320% w 17.854%) / `space`(24.707%, 26.781%) / `left`(53.021%, 8.656%) / **`up`(61.858%, 6.853%, top 79.219% h 4.660%)** / **`down`(61.858%, 6.853%, top 85.139% h 4.660%, up과 사이 gap 1.26%p — 세로로 확실히 분리)** / `right`(70.243%, 8.656%) / `enter`(79.080%, 15.690%).

### 4. Shift 조합 확장 — ㅔ→ㅖ
`hangul.js`의 `JUNG` 배열엔 이미 `ㅖ`가 있어 조합 오토마타 자체는 변경 불필요. `js/app.js`의 `SHIFT_VOWEL`에 `'ㅔ':'ㅖ'` 추가(`{'ㅐ':'ㅒ'}` 기존 항목에 병기). 기존 "이 기기엔 ㅔ 키가 없어 ㅖ 미지원"이라는 정직한 한계 기록은 이번 이미지 교체로 더 이상 유효하지 않음 — `CLAUDE.md`에서도 해당 문구 갱신.

### 5. ▲/▼ 히트존 분리 (사용자 보고 버그 수정)
기존 코드는 R5의 up/down을 "한 칸을 세로로 절반씩 기계적으로 분할"하는 방식이었다(`R5_H/2`). 이 기기 사진에서는 ▲/▼가 애초에 세로로 분리된 별개 버튼이고 둘 사이에 여백(gap)이 있으므로, 기계적 절반分割 대신 **각자의 실제 bbox를 개별 실측**해 히트존을 배치(▲ top 79.219~83.879%, ▼ top 85.139~89.798%, 그 사이 1.26%p gap). 새 kr_trim 이미지에서는 이 방식이 아니어도 절반分割과 결과가 크게 다르지 않았지만(원본 사진 자체가 이미 정확히 반반에 가깝게 배치돼 있었음), 실측 기반으로 바꿔 향후 다른 사진으로 교체되어도 안전하도록 근본적으로 고쳤다. 이 좌표는 한글/영문/이모지 세 레이어가 공유(R5는 R1과 마찬가지로 공용 좌표).

### 6. 챠시 정합 확인 (LCD 위치 큰 어긋남 발견 및 반영)
새 kr_trim의 LCD/슬롯/핑크버튼/오벌 위치를 en_trim(1095×790)/emoji_trim(1095×792)과 공유하던 기존 CSS 값과 대조:
- **슬롯**: left 19.116% top 4.912% width 61.858% height 0.630% — 기존(18.43%/6.19%/62.5%/0.76%) 대비 오차 1.3%p 이내, 경미. CSS 갱신.
- **핑크버튼**: center(87.376%, 18.829%) 지름(11.362%, 15.995%) — 기존(87.409%/20.013%/11.770%/16.162%) 대비 오차 1%p 이내, 경미. CSS 갱신.
- **오벌 5개**: left들이 26.69/36.61/46.44/56.36/66.37%로 기존(26.46/36.22/46.17/56.11/65.97%)과 오차 0.2~0.4%p, 경미. top/height는 pill 코어 기준 재실측시 32.620%/5.164%로 기존(33.838%/4.545%)과 오차 1.2%p/0.6%p, 경미. CSS 갱신.
- **LCD — 크게 어긋남(보고 대상)**: 새 kr_trim 실측 left 26.510% top 12.406% width 47.160% height 12.531%. 기존 값(left 25.365% top 6.944% width 55.383% height 30.303%)과 비교하면 **height가 30.303%→12.531%로 절반 이하(약 2.4배 차이)**, width도 55.383%→47.160%(8.2%p 차이) — 새 이미지의 LCD 화면 자체가 기존보다 훨씬 가늘고 작은 슬림 스크린이다(육안으로도 확인: 기존 이미지는 두꺼운 사각 LCD, 새 이미지는 얇고 넓적한 LCD). `.machine-lcd`를 새 실측값으로 전면 교체.
- 좌우(x) 방향은 대체로 1%p 이내로 정합했지만 top/height(y) 방향에서 LCD만 유독 크게 다름 — 이미지 자체의 실제 시각 디자인이 달라진 것으로 판단(코드 실수 아님).

### 7. R1/R5 공용 좌표의 챠시 간 미세 오차 (정직하게 명시)
R1/R5는 한/영/이모지 세 레이어가 공유하는데, kr_trim(1109×794, 비율 1.3967)이 en_trim(1095×790, 1.3861)/emoji_trim(1095×792, 1.3826)과 비율이 약 0.75~1.0%p 다르다. 각 레이어 전환 시 `machineImgEl.src`가 바뀌면서 오버레이 %가 그 이미지 자체 크기에 다시 대응되므로 히트존이 "엉뚱한 키"로 밀리는 구조적 문제는 없지만, 픽셀 스캔 정밀 검증에서 R1의 "7"(en_trim), "H"(en_trim) 두 키가 코어 크림색이 아닌 안티에일리어싱 경계색(214,207,195 / 230,218,203)에 걸려 자동 판정상 "실패"로 나왔다 — 그러나 실제 클릭 이벤트 테스트(`elementFromPoint`+`dispatchEvent`)로는 두 키 모두 정확히 자기 자신에 히트하고 올바른 문자(`7`, `h`)가 삽입됨을 재확인해, 기능적으로는 문제없음을 확인했다.

### 검증 (preview_eval, 단일 세션 루프로 전체 실행 — 페이지 리로드 없이 진행)
- **letter 키 39개 전수 클릭**(숫자10 + R2 10자모 + R3 9자모+BS 제외 + R4 7자모+콤마2+마침표): `elementFromPoint`로 자기 자신 히트 확인(`isSelf:true`) 후 실제 클릭 이벤트로 삽입된 글자가 기대값과 정확히 일치 — **39/39 (100%)**.
- **에 = ㅇ+ㅔ**: `"에"` 정확히 조합됨.
- **Shift+ㅔ → ㅖ**: 정확히 확인. **Shift+ㅐ → ㅒ**(회귀 확인): 정확히 유지됨.
- **▲/▼ 분리**: `getBoundingClientRect()` 기준 겹침 없음(`upDownOverlap:false`), 각각 `elementFromPoint`로 자기 자신 히트(`upHitIsSelf`/`downHitIsSelf:true`). 실제 기능 테스트(환경 오벌 진입 → ▼ 클릭 → 카테고리 "테이프색"→"프레임" 이동 → ▲ 클릭 → "테이프색"로 복귀) 정상 동작. ◀/▶(왼쪽/오른쪽 이동)도 자기 자신 히트 확인.
- **키캡 중심 통과율**: 한글 레이어 50개 히트존 전수 픽셀 스캔 — 49/50 자동 판정 통과, 실패 1건(Shift, RGB 203,226,172)은 Shift 버튼 고유의 초록 강조색을 "노란 데크"로 오분류한 측정 함수의 한계이지 실제 오차 아님(좌표 자체는 `R4_KR_RECTS`에서 정확히 계산된 값) — 실질 **50/50(100%)**.
- **375px**: `scrollWidth===clientWidth===innerWidth===375`(가로 스크롤 없음). 에(ㅇ+ㅔ) 재확인 정상. ▲/▼ 겹침 없음 재확인.
- **한/영/이모지 레이어 회귀**: 한→영 전환 시 `en_trim.png` 로드 + Q키 정상(`q` 삽입), 영→한 복귀 정상. 한→이모지 전환 시 `emoji_trim.png` 로드(49개 존, 기존과 동일), 이모지 레이어에서도 ▲/▼ 안 겹침 재확인, "가나다 복귀"로 `kr_trim.png` 복귀 정상.
- 콘솔 에러 0건, 네트워크 실패 0건.
- **LCD/키보드/오벌/핑크버튼 배치**: `getBoundingClientRect()` 기준 LCD가 오벌 행·핑크버튼과 겹치지 않고(`lcdOverlapsOvals:false`, `lcdOverlapsPrintBtn:false`) 기기 안에 완전히 포함(`lcdWithinMachine:true`).

### 직접 확인 못 한 부분 (정직하게 명시)
- **육안 스크린샷 검증 불가**: 지시("preview_eval만")에 따라 `preview_screenshot`을 쓰지 않았다(시도 시 실제로 30초 타임아웃 발생, 지시와도 일치해 재시도하지 않음). LCD가 실제로 "얇고 넓적해 보기 불편하지 않은지", 새 키보드 사진의 실제 색감·질감이 기존 UI와 조화로운지는 좌표·픽셀 스캔 수치로만 검증했다.
- **R1/R5 공용 좌표의 en_trim/emoji_trim 챠시 간 미세 오차(약 1%p)**: 기능적으로 문제없음을 클릭 테스트로 확인했으나, 안티에일리어싱 경계 정도의 미세한 시각적 밀림이 있을 수 있다(육안 미확인).
- **오버레이 그리드 이미지 육안 대조는 Read 도구로 정적 PNG 파일을 열어 진행**(스크린샷이 아니라 Python으로 생성한 그리드 오버레이 이미지) — 매우 높은 확신도로 격자 위치를 검증했으나, 실제 브라우저 렌더링에서의 안티에일리어싱·서브픽셀 차이까지는 이 방식으로 완전히 재현하지 못한다.

## 2026-07-05(11차) — 이모지 키보드 히트존 좌표 근본 재측정 (kr 좌표 재사용 버그 수정)

### 문제 (사용자 보고)
이모지 레이어에서 (1) 화면에 그려진 이모지 키와 클릭 히트존 위치가 안 맞고, (2) 그 결과 누른 키와 다른 이모지가 삽입됨.

### 원인
2026-07-05(10차)까지 코드는 R2/R3/R4 이모지 히트존 좌표로 **한글 자판(kr_trim.png)의 좌표(`R2_KR_RECTS` 등)를 그대로 재사용**했다. "히트존 중심이 키캡 위(노란 데크 아님)에 있는지"만 검산했는데, 이 검산은 칸 개수가 달라도 우연히 통과할 수 있다 — 실제로 `assets/emoji_trim.png`를 kr_trim.png와 별개로 Python PIL로 재측정한 결과:
- **R2**: 한글은 9자모+토글=10칸이지만, 이모지는 **11칸**(곰·고양이·토끼·딸기·꽃·하트·반짝이×3종·리본·토글).
- **R3**: 10칸으로 개수는 같았으나 폭 배분이 다름(9번째는 빈칸, 사진에 아이콘 없음).
- **R4**: 한글은 Shift+7자모+`,`+`.`=10칸이지만, 이모지는 **11칸**(Shift+이모지8종+`,`+`.`).
칸 개수 자체가 다른데 10칸짜리 좌표를 11칸 그림 위에 얹었으니 뒤쪽 키일수록 누적 오차로 히트존이 옆 키로 밀려 있었다 — 이게 "그려진 키와 히트존이 안 맞고 엉뚱한 이모지가 삽입된다"는 증상의 근본 원인.
또한 기존 검증은 전부 `aria-label="이모지 삽입"`으로 버튼을 직접 찾아 클릭하는 방식이라, 히트존이 실제 그려진 키 위에 있는지는 한 번도 검증된 적이 없었다(좌표가 밀려 있어도 aria-label 클릭은 항상 "성공"으로 보였다).

### 해결
`assets/emoji_trim.png`(1095×792)를 kr 좌표와 완전히 독립적으로 재측정했다(측정 스크립트는 1회성이라 저장하지 않음).
- **세로(top/height)**: R2/R3/R4 각 행 경계를 다수결 픽셀 스캔(크림 키캡 vs 노란 데크, x 30~1065 사이 8px 간격 샘플)으로 재확인한 결과 kr_trim과 오차 0.3% 이내로 사실상 동일 — top/height는 kr 값 그대로 사용.
- **가로(칸 개수·폭)**: 각 행에서 "크림(non-deck) 연속 구간"을 열 방향으로 스캔해 칸 경계를 직접 검출. R2=11칸, R3=10칸(9번째 빈칸), R4=11칸으로 확정.
- **그림 식별**: 각 키캡 영역에서 잉크 픽셀(밝기<150) bbox를 크롭해 `assets/emoji/*.png` 17개 스프라이트와 나란히 놓고 시각 대조(자동 IoU 매칭은 아웃라인 두께 차이로 신뢰도가 낮아 보조 신호로만 사용, 최종 확정은 육안 대조로 함).

**최종 확정 좌표/매핑**(emoji_trim.png 기준 %, top/height는 kr과 공용):
- R2(top 51.768 h 7.449, 11칸): `bear`(6.484,7.123) `cat`(15.068,6.941) `rabbit`(23.470,6.849) `strawberry`(31.872,6.758) `flower`(40.183,6.758) `heart`(48.402,6.667) `sparkle_pair`(56.530,6.393) `sparkle_small`(64.384,5.936) `sparkle_big`(71.689,6.210) `ribbon`(79.269,6.027) `emoji토글`(86.667,6.941)
- R3(top 60.985 h 7.449, 10칸): `mug`(6.484,7.489) `cloud`(15.616,7.763) `note`(25.023,7.580) `moon`(34.338,7.945) `smiley`(43.927,7.945) `star`(53.607,7.397) `sparkle_small`(62.648,6.758) `heart_outline`(71.050,6.667) `EMPTY`(77.900,8.402, 히트존 없음) `backspace`(86.393,7.215)
- R4(top 70.202 h 7.449, 11칸, 전부 mini): `shift`(5.114,11.050, 비활성) `sparkle_big`(16.256,6.301) `ribbon`(23.836,6.301) `flower`(31.507,6.119) `bear`(39.087,6.119) `cat`(46.575,6.210) `rabbit`(54.155,6.210) `strawberry`(61.735,6.210) `cloud`(69.315,6.301) `,`(77.169,7.489) `.`(86.119,7.489)

**10차 대비 변경점**: R2에 `sparkle_big`(별4점) 1칸 추가로 10칸→11칸(반짝이가 2종이 아니라 3종이었음). R4에 `sparkle_small`→`sparkle_big`으로 정정 + `,`가 `cloud`에 흡수되지 않고 별도 칸으로 분리되어 10칸→11칸.

`js/app.js`의 `R2_EMOJI_RECTS`/`R3_EMOJI_RECTS`/`R4_EMOJI_RECTS`를 신설(기존엔 `R2_KR_RECTS` 등을 그대로 참조)하고 `R2_EMOJI_KEYS`/`R4_EMOJI_KEYS`를 위 매핑으로 재작성. `EMOJI_SPRITES`(17개 스프라이트 경로 맵)는 변경 없음 — 이미 `sparkle_big` 포함.

### 검증
- **검산 A(위치)**: 26개 이모지 히트존 전부 중심 픽셀이 emoji_trim.png의 크림 키캡 위(노란 데크 아님) — canvas 픽셀 스캔, **26/26 (100%)**. 데스크톱/375px 둘 다 확인.
- **검산 B(그림-스프라이트 일치)**: 26개 키캡 크롭과 대응 스프라이트를 나란히 이어붙인 비교 이미지로 육안 대조 — 전부 형태 일치 확인(곰/고양이/토끼/딸기/꽃/하트/반짝이 3종/리본/머그컵/구름/음표/달/스마일/별/하트외곽선 전부).
- **검증 C(핵심, 그려진 키 직접 클릭)**: aria-label이 아니라 실측한 각 키 bbox의 **화면 좌표 중심**을 계산해 `document.elementFromPoint(x,y)`로 그 지점의 실제 요소를 찾아 클릭 → 삽입된 스프라이트 src가 그 좌표에 그려진 이모지와 일치하는지 확인. **26/26 전부 일치**(데스크톱/375px 둘 다). 이전 세션들이 못 채운 검증 허점(aria-label 클릭은 좌표가 밀려도 항상 성공으로 보임)을 이번에 메웠다.
- R4 mini 사이즈 유지 확인(`.lcd-emoji--mini` 클래스 부여), R3 9번째 빈칸은 히트존 자체가 없음(`elementFromPoint`가 keyzone 아닌 다른 요소 반환) 확인, Shift 비활성(`disabled:true`) 확인, 콤마/마침표 정상 입력 확인, "가나다 복귀" 토글로 `kr_trim.png` 복귀 + 텍스트 버퍼 보존 확인.
- 스팟 체크: 한/영 전환(`en_trim.png`↔`kr_trim.png`) 정상, 환경 오벌(`is-modal` 진입) 정상 — 회귀 없음.
- 콘솔 에러 0건, 네트워크 실패 0건(데스크톱/375px 둘 다). 375px `scrollWidth===clientWidth===innerWidth===375`(가로 스크롤 없음).

### 직접 확인 못 한 부분 (정직하게 명시)
- **육안 스크린샷 검증 불가**: 지시에 따라 `preview_screenshot`을 쓰지 않았다. 다만 이번엔 Python으로 크롭한 키캡 이미지와 스프라이트를 나란히 붙인 비교 이미지를 Read 도구로 직접 열어 육안 대조했으므로(스크린샷이 아니라 정적 이미지 파일), 매핑 자체의 시각적 정확성은 높은 확신도로 검증했다. 다만 실제 브라우저 화면에서 키 눌림 피드백(`::after` 오버레이)이 정확한 키 위치에 뜨는지, LCD에 삽입된 이모지가 "귀여워 보이는지"는 육안 미확인.
- **자동 IoU/정규화 상관 매칭은 신뢰도가 낮았음**: 스프라이트(가는 아웃라인)와 키캡 그림의 안티에일리어싱/선 두께 차이로 자동 매칭 점수가 0.06~0.26 수준으로 낮게 나와 단독 근거로 쓰지 못했다. 최종 확정은 크롭-스프라이트 나란히 비교 이미지의 육안 대조로 했다(검산 B에 기록).
